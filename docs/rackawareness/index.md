...
When [well-known Kubernetes labels](https://kubernetes.io/docs/reference/kubernetes-api/labels-annotations-taints/) are available (for example, AZ, node labels, and so on), {{< kafka-operator >}} attempts to improve broker resilience by default.
...
As mentioned earlier, `broker.rack` is a read-only broker config, so is set whenever the broker starts or restarts. [{{< kafka-operator >}}](https://github.com/banzaicloud/koperator) holds all its configs within a ConfigMap in each broker.
