...
When the Kafka cluster is upscaled or downscaled with a broker or when a new storage is added for a broker, Koperator creates a `CruiseControlOperation` custom resource. This created custom resource describes a task what will be executed with the Cruse Control to do the partitions movements. When there are multiple `CruiseControlOperation` in the same time Cruise Control does not able to execute more then one task thus there is a priority between them depending their operation type. Upscale operations will be exetuced first then the donwscale operations and after disk rebalance operations. Koperator watches this created `CruiseControlOperation` custom resources and updates their state based on the Cruise Control task result. Koperator can re-execute the task when the task has failed. The process can be followed up through the `KafkaCluster` custom resource status and the created `CruiseControlOperation` custom resource status. In the above section you can see an example for the upscale (`GracefulUpscale*`) operation but the same applies for the Kafka cluster downscale (`GracefulDownScale*`) and for the volume rebalance (when the `volumeState` is `GracefulDiskRebalance*`) operations.
...
4. When the Cruise Control task is completed:
